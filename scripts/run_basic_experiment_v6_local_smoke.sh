#!/usr/bin/env bash
# One object/contact set per hand: real v6 checkpoint -> AR -> FK -> EAWQ -> PhysX.
set -euo pipefail

project_root="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
mck_root="$(cd "${project_root}/.." && pwd)"
python_path="${CONTACT_V6_PYTHON:-/home/zhb1/miniconda3/envs/Dexgrasp/bin/python}"
isaac_runner="${CONTACT_V6_ISAAC_RUNNER:-${mck_root}/IsaacGymLocal/run_python_cpu.sh}"
checkpoint="${CONTACT_V6_CHECKPOINT:-${project_root}/weights/v6/step_00056000.pt}"
protocol="${project_root}/configs/basic_experiment_mixed_full_partial_ar56k_eawq_o10i20_palm0_v6_smoke1_protocol.yaml"
manifest="${project_root}/configs/basic_experiment_ood10_local_manifest.json"
run_root="${CONTACT_V6_SMOKE_ROOT:-${project_root}/outputs/v6_local_end_to_end_smoke_verified}"
object_id="${CONTACT_V6_SMOKE_OBJECT:-contactdb_apple}"
device_id="${CONTACT_V6_SMOKE_DEVICE_ID:-0}"
hand_cooldown_seconds="${CONTACT_V6_SMOKE_HAND_COOLDOWN_SECONDS:-10}"
expected_sha="05bd542bb0c10a74ed8dafd7586a58ceedb17097dd08eb344011bb91e98516b4"
...
summary = {"schema": "contactdiff-v6-local-smoke-v1", "run_root": str(root.resolve())}
